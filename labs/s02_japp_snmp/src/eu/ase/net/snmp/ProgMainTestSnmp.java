...
public class ProgMainTestSnmp {
	...
	public static void main(String[] args) {
		...
		try {
			...
			byte[] bufRecv1 = new byte[256];
			packet = new DatagramPacket(bufRecv1, bufRecv1.length);
			socket.receive(packet);
			
			byte[] bufRecv2 = packet.getData();
			if(bufRecv1 == bufRecv2) {
				System.out.println("TRUE - buf1 == buf2");
			}
			
			for(int i = 0; i < bufRecv2.length; i++)
				System.out.printf(" %02x", bufRecv2[i]);
